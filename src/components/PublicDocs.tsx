...
export const AboutPage = () => <DocLayout title="サービス概要" lead="Stock League Classroom は、教室で使うことを目的とした株式市場のシミュレーターです。">
  ...
  <aside className="doc-callout warning" role="note">
    <strong>同時に開催できる教室はおおむね1つです。</strong>
    <p>本サービスは無償の小規模な基盤で運営しており、全利用者で共有できる同時接続数に上限があります。他の教室が授業中の場合、参加できなかったり、授業の途中で接続が切れたりすることがあります。<strong>本サービスの動作を前提としない授業計画をご用意ください。</strong>確実に使いたい日時がある場合は、事前に問い合わせ窓口へご連絡ください。</p>
  </aside>
...
export const GuidePage = () => <DocLayout title="教師向け操作マニュアル" lead="授業の準備から振り返りまでの手順です。">
  ...
  <h2>3. 生徒の参加を承認する</h2>
  <p>生徒が参加コードを入力すると承認待ちに並びます。チーム編成の方式（自動割当・希望優先・手動）を選び、一人ずつ承認してください。定員は80名です。参加は授業の途中でも受け付けられるので、遅れて来た生徒がいても参加コードを伝えて承認すれば合流できます。</p>
  <p>次の2点は、授業の最初に生徒へ伝えておいてください。</p>
  <ul>
  ...
  <h2>5. 振り返り</h2>
  <p>生徒は自分の市場画面で取引履歴と結果を確認できます。「なぜその判断をしたか」を言語化させる時間を取ると効果的です。教師は管理画面の「結果をCSVで保存」から、チーム結果と取引履歴の2つのファイルを保存できます。</p>
  <h2>困ったとき</h2>
  <ul>
    <li><strong>生徒が「市場につながりません」と表示される</strong> — 他の教室が同時に授業中で、同時接続数の上限に達している可能性があります。しばらく待つと自動で復帰します。売買した内容は保存されているため、続きから再開できます</li>
    <li><strong>生徒が参加できない</strong> — 参加コードの入力間違いの可能性があります。参加は授業の途中でも受け付けられるので、市場が始まっていることは理由になりません。参加コードと表示名をもう一度確認してもらってください</li>
