...
export const HostConsole = ({ marketId }: { marketId: string }) => {
  ...
  useEffect(() => {
    ...
    const tick = () => void runHostTick(services.firestore, services.database, marketId, user.uid, lease, stocks)
      .then((ok) => { if (ok) setLastTickAtMillis(serverNow()); else { setLease(''); setLastTickAtMillis(undefined); setHostingSinceMillis(undefined); setNotice(liveRef.current?.meta?.status === 'ENDED' ? '終了処理が完了しました。結果は確定しています。' : 'ホストリースが失効しました。もう一度「ホストを取得する」を押してください。') } })
