...
export const HostConsole = ({ marketId }: { marketId: string }) => {
  ...
  return <main><h1>ホストコンソール</h1><p>{notice}</p><button type="button" onClick={() => void takeLease().catch(() => setNotice('ホスト取得に失敗しました。'))}>ホストを取得</button>{lease && <><button type="button" onClick={() => void openMarket(services.database, marketId, user.uid, lease).then(() => setNotice('市場を開始しました。')).catch(() => setNotice('開始できません。'))}>市場を開始</button><button type="button" onClick={() => { if (window.confirm('市場を終了し、結果を確定しますか？')) void requestMarketEnding(services.database, marketId, user.uid, lease).then((result) => setNotice(result.committed ? '終了処理を開始しました。完了まで再試行します。' : '終了処理を開始できません。')) }}>市場を終了</button><label>手動ニュース <input value={news} onChange={(event) => setNews(event.target.value)} /></label><button type="button" onClick={() => void publishManualNews(services.database, marketId, user.uid, lease, news).then(() => { setNews(''); setNotice('ニュースを配信しました。') }).catch(() => setNotice('ニュースを配信できません。'))}>配信</button></>}</main>
}
