...
export const HostConsole = ({ marketId }: { marketId: string }) => {
  ...
  useEffect(() => {
    ...
    const tick = () => void runHostTick(services.firestore, services.database, marketId, user.uid, lease, stocks)
      .then((ok) => { if (ok) setLastTickAtMillis(serverNow()); else { setLease(''); setLastTickAtMillis(undefined); setHostingSinceMillis(undefined); setNotice('ホストリースが失効しました。もう一度「ホストを取得する」を押してください。') } })
