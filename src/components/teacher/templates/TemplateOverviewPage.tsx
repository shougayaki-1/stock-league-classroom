...
export interface TemplateOverviewPageProps { answers: WizardAnswers; onCreate: (draft: LessonContent) => void; creating: boolean; functions: Functions; aiEnabled: boolean }
export function TemplateOverviewPage({ answers, onCreate, creating, functions, aiEnabled }: TemplateOverviewPageProps) {
  ...
  const chooseAi = async () => {
    ...
    } catch { setAiError('AI提案の生成に失敗しました。固定の案をご利用ください。') } finally { setAiLoading(false) }
  }
  if (!chosen) return <Stack spacing={2}><Typography variant="h6">3つの案から選んでください</Typography>{aiError && <Alert severity="warning">{aiError}</Alert>}<Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>{drafts.map(({ tier, draft }) => <Card key={tier} sx={{ flex: 1 }}><CardActionArea aria-label={`${labels[tier]}を選ぶ`} onClick={() => setChosen(draft)}><CardContent><Typography sx={{ fontWeight: 700 }}>{labels[tier]}</Typography><Typography variant="body2">{draft.title}</Typography></CardContent></CardActionArea></Card>)}{aiEnabled && <Card sx={{ flex: 1 }}><CardActionArea aria-label="AI提案を選ぶ" onClick={chooseAi} disabled={aiLoading}><CardContent><Typography sx={{ fontWeight: 700 }}>AI提案</Typography><Typography variant="body2">条件に合わせたタイトルと説明を提案します。</Typography>{aiLoading && <CircularProgress size={20} aria-label="AI提案を生成中" />}</CardContent></CardActionArea></Card>}</Stack></Stack>
