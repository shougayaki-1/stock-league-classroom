...
describe('MaterialUploadPanel', () => {
  it('lists materials and toggles selection', () => {
    ...
    render(<MaterialUploadPanel materials={materials} uploading={false} onUpload={vi.fn()} selectedIds={[]} onSelectionChange={selected} />)
  ...
  it('uploads the selected file and has an empty state', () => {
    const upload = vi.fn()
    render(<MaterialUploadPanel materials={[]} uploading={false} onUpload={upload} selectedIds={[]} onSelectionChange={vi.fn()} />)
    fireEvent.change(screen.getByLabelText('資料をアップロード'), { target: { files: [new File(['x'], 'new.pdf', { type: 'application/pdf' })] } })
