...
describe('TemplateEditorPage', () => {
  ...
  it('edits and saves an added household', () => {
    const save = vi.fn()
    render(<TemplateEditorPage {...props} aiEnabled={false} onSaveDraft={save} />)
    fireEvent.click(screen.getByRole('tab', { name: '主要な一覧' }))
    fireEvent.click(screen.getByRole('button', { name: '担当プロフィールを追加' }))
    fireEvent.click(screen.getByRole('button', { name: '下書き保存' }))
    expect(save.mock.calls[0][0].homeEconomics.households.length).toBe(2)
  })

  it('updates editable title and description from selected materials without saving when APPROVED', async () => {
    const save = vi.fn()
    vi.mocked(listMaterials).mockResolvedValue([{ id: 'm1', fileName: '資料.pdf', text: '内容' }])
    vi.mocked(generateLessonDraft).mockResolvedValue({ title: 'AIタイトル', description: 'AI説明' })
    render(
      <TemplateEditorPage
        {...props}
        aiEnabled
    ...
    await waitFor(() => expect(screen.getByLabelText('タイトル')).toHaveValue('AIタイトル'))
    fireEvent.click(screen.getByRole('tab', { name: '基本情報' }))
    expect(screen.getByLabelText('説明')).toHaveValue('AI説明')
    expect(save).not.toHaveBeenCalled()
  })

  it('shows existing materials in LOCKED state but disables upload and AI regeneration', async () => {
    vi.mocked(listMaterials).mockResolvedValue([{ id: 'm1', fileName: '既存資料.pdf', text: '内容' }])
    render(
      <TemplateEditorPage
        {...props}
        aiEnabled
    ...
    fireEvent.click(screen.getByRole('tab', { name: '資料' }))
    await waitFor(() => expect(screen.getByText('既存資料.pdf')).toBeInTheDocument())
    ...
    expect(screen.getByRole('button', { name: /資料をアップロード/ })).toHaveAttribute('aria-disabled', 'true')
    expect(screen.getByRole('button', { name: '資料を使ってAI提案を更新' })).toBeDisabled()
  })

  it('shows error notice and disables upload and AI regeneration when aiBetaState is ERROR', async () => {
    vi.mocked(listMaterials).mockResolvedValue([{ id: 'm1', fileName: '既存資料.pdf', text: '内容' }])
    render(
      <TemplateEditorPage
        {...props}
