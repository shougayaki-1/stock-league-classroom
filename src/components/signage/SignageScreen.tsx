...
export function SignageScreen({ data, joinUrl }: SignageScreenProps) {
  return (
    <Box component="main" sx={{ minHeight: '100svh', p: { xs: 2, md: 4 }, bgcolor: 'background.default' }}>
      <Stack component="header" direction="row" sx={{ mb: 3, justifyContent: 'space-between', alignItems: 'center' }}><Box><Typography variant="overline" color="primary.main">STOCK LEAGUE CLASSROOM</Typography><Typography variant="h1">教室マーケット</Typography></Box><Chip color={data.phase === 'OPEN' ? 'success' : 'default'} label={data.phase} sx={{ fontWeight: 700 }} /></Stack>
