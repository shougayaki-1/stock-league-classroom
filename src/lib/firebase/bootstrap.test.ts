...
describe('Firebase bootstrap', () => {
  it('initializes services and emulator connections once before consumers', () => {
    const services = { app: {} as never, auth: {} as never, firestore: {} as never, database: {} as never }
  ...
  it('does not connect emulators unless explicitly requested', () => {
    const bootstrap = createFirebaseBootstrapper({ getServices: () => ({ app: {} as never, auth: {} as never, firestore: {} as never, database: {} as never }), connectToEmulators: vi.fn(), initializeAppCheck: vi.fn(() => undefined) })
