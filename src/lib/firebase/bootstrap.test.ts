...
describe('Firebase bootstrap', () => {
  it('initializes services and emulator connections once before consumers', () => {
    const services = { app: {} as never, auth: {} as never, firestore: {} as never, database: {} as never, functions: {} as never, storage: {} as never }
    ...
    expect(connectToEmulators).toHaveBeenCalledWith(services.auth, services.firestore, services.database, services.functions, services.storage)
  })
  it('does not connect emulators unless explicitly requested', () => {
    const bootstrap = createFirebaseBootstrapper({ getServices: () => ({ app: {} as never, auth: {} as never, firestore: {} as never, database: {} as never, functions: {} as never, storage: {} as never }), connectToEmulators: vi.fn(), initializeAppCheck: vi.fn(() => undefined) })
