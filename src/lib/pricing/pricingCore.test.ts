...
describe('pricing core', () => {
  ...
  it('selects a phase by minutes elapsed since market open', () => { const p: StockPricePhase[] = [{ id: 'a', startMinute: 0, endMinute: 30, direction: 'FLAT', changePercent: 0 }, { id: 'b', startMinute: 30, endMinute: 60, direction: 'UP', changePercent: 5 }]; expect(getActivePhase(p, 40).id).toBe('b') })
  it('creates Firebase-independent runtime', () => { const p: StockPricePhase = { id: 'f', startMinute: 0, endMinute: 60, direction: 'FLAT', changePercent: 0 }; expect(getPhaseEndPrice(1000, p)).toBe(1000); const opened = Date.now(); const runtime = createPhaseRuntime(1000, p, opened, opened, 1000, 42); expect(runtime.endAtMillis).toBeGreaterThan(runtime.startAtMillis); expect(runtime.seed).toBe(42) })
})
