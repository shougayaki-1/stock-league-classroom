...
const makeFakeFirestore = () => {
  ...
  return {
    ...
    // any `get` after a `set`/`update` within that same transaction throws.
    // `restoreCheckpoint` (get, get, get, then appendLessonEventInTransaction's
    // own get/set, then update/set) already follows the correct
    // read-phase-then-write-phase ordering, so this guard should pass
    // without any production code change here.
